...
export class ArticleController {
  ...
  /**
   * Updates an existing article identified by its slug. Extracts the title, description, and content from the request body,
   * and the author's ID from the authenticated user. If files are uploaded, they are processed by `imageService.saveImages`
   * to save under a directory named after the article's slug.
  ...
  public async updateArticle(req: Request, res: Response): Promise<void> {
    ...
    const article = await articleRepository.findBySlugAndUpdate(slug, title, description, content, author, { session: req.session })
    if (!article) {
